//CodePen https://codepen.io/lerida/pen/oNjdPGq
...
const PassStrengthViz = ({ password }) => {
  const swordLeftRef = useRef(null);
  const swordRightRef = useRef(null);
  ...
  useEffect(() => {
    ...
    swordLeftRef.current = gsap.timeline({ paused: true });
    swordLeftRef.current.to("#sword-left", .5, { opacity: 1, rotate: 0, y: 60, x: 0, ease: "back.out" });

    swordRightRef.current = gsap.timeline({ paused: true });
    swordRightRef.current.to("#sword-right", .5, { opacity: 1, rotate: 0, y: 60, x: 0, ease: "power4.inOut" });

    wingsRef.current = gsap.timeline({ paused: true });
    wingsRef.current.to("#wings", .5, { delay: .1, opacity: 1, scale: 1, y: 0, ease: "back.out", transformOrigin: "center" });

    crownRef.current = gsap.timeline({ paused: true });
    crownRef.current.to("#crown", .5, { delay: .1, opacity: 1, y: 0, rotate: 0, ease: "back.out" });
}, []);
  ...
  useEffect(() => {
    ...
    if (passStrength > 0) swordLeftRef.current.play();
    else swordLeftRef.current.reverse();

    if (passStrength > 1) swordRightRef.current.play();
    else swordRightRef.current.reverse();

    if (passStrength > 2) wingsRef.current.play();
    else wingsRef.current.reverse();

    if (passStrength > 3) crownRef.current.play();
    else crownRef.current.reverse();
  }, [password]);
